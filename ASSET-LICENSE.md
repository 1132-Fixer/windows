<!-- This notice supplements the repository's MIT declaration (package.json, "license": "MIT") and the end-user distribution terms in build/license.txt. It modifies neither. This repository has no standalone root LICENSE file; the MIT declaration in package.json is the source-license statement this notice qualifies, prospectively from this notice forward. -->
...
The MIT declaration in [`package.json`](package.json) (`"license": "MIT"`) covers the
code, the documentation, and the eligible design tokens in this repository. That
...
Distributed installers and portable builds are governed by the end-user terms in
[`build/license.txt`](build/license.txt), which this notice does not modify.
